...
| **序号** | **功能点** | **描述** | **备注** |
| -------- | ---------- | -------- | -------- |
| 1 | 设置从实例流向外网的最大带宽 | 1. 允许用户设置实例出外网的最大带宽（即下行最大带宽）。 <br/>2. 允许用户设置的最大带宽范围为：[A, B]（A 和 B 为正整数，且 A < B，单位：Mbps），且只能为整数。 <br/>3. 不允许用户设置实例入外网的最大带宽，即上行带宽不做限制。 <br/>4. 上行带宽不能影响下行带宽。[见各云厂商的带宽限制情况。](http://note.youdao.com/) <br/>5. 内网之间的通信没有带宽限制。| |
